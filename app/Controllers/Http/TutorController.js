...
class TutorController{
  ...
  async update({request, response, params}){
    try{
      ...
      const dataTutor = request.only(['avatar', 'document', 'email', 'description']);
      const dataUser = request.only(['username', 'password', 'role', 'profession']);
      ...
      try {
        ...
        dataTutor.document = (dataTutor.document)?dataTutor.document : tutor.document;
        dataTutor.email = (dataTutor.email)? dataTutor.email :tutor.email;
        dataTutor.description = (dataTutor.description)?dataTutor.description : tutor.description;

        tutor.merge(dataTutor);
        await tutor.save(trx);

        dataUser.username = dataUser.username ? dataUser.username : user.username;
        dataUser.password = dataUser.password ? dataUser.password : user.password;
        dataUser.role = dataUser.role ? dataUser.role : user.role;
        dataUser.profession = dataUser.profession ? dataUser.profession : user.profession;
        dataUser.tutor_id = 3;
